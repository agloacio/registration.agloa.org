<?php
/**
 * This file allows the plugin to update itself.
 *
...
class Crio_Premium_Updater {
	...
	public function __construct() {
		$this->configs = apply_filters(
			'bgcriop_update_configs',
			array(
				'ajax_calls'            => array(
				...
				'plugin_name'           => 'crio-premium',
				'plugin_key_code'       => 'crio-premium',
				'main_file_path'        => CRIO_PREMIUM_FILEPATH,
				'plugin_transient_name' => 'bgcriop_version_data',
			)
		);
	}
	...
	public function custom_plugins_transient_update( $transient, $action, $args = array() ) {
		...
		$is_force_check = isset( $_GET['force-check'] ); // phpcs:ignore WordPress.CSRF.NonceVerification
		...
		if ( 'plugin-install.php' === $pagenow && isset( $_GET['plugin'] ) && // phpcs:ignore WordPress.CSRF.NonceVerification
			$this->configs['plugin_name'] === $_GET['plugin'] ) { // phpcs:ignore WordPress.CSRF.NonceVerification
	...
	public function site_transient_update_plugins( $value ) {
		...
		if ( 'update-core.php' !== $pagenow || ! isset( $_GET['force-check'] ) ) { // phpcs:ignore WordPress.CSRF.NonceVerification
			return $value;
		}
